...
function checkCCDetails(ccNumber: number[]) {
    ...
    // Take first digit to determine the issuing industry
    let firstDigit: number = ccNumber[0];

    // Take the first 6 digits for card type determination
    let first6Digits: number[] = ccNumber.slice(0, 6);

    // Take digits 7-15, which usually relate to the account number
