...
function makeCCNumber() {
    let newCC = []; // create an array to store the credit card number digits
    let LuhnSum = 0; // create a variable to store the Luhn sum
    // create a constant for accessing the paragraph for displaying results
    ...
    for (let i = 0; i < 15; i++) {
        let digit = newCC[i]; // get the digit at the current index
    ...
    if (lastDigit < 0 || lastDigit > 9) {
        ...
        // add the last digit to the credit card number
...
function checkCCDetails(ccNumber) {
    ...
    // Define industry categories based on the first digit
    let firstDigitInfo = [
        "Airlines",
        "Airlines",
    ...
    // Extract first digit to determine the issuing industry
    let firstDigit = ccNumber[0];
    // Extract the first 6 digits for card type determination
    let first6Digits = ccNumber.slice(0, 6);
    // Extract digits 7-15, which usually relate to the account number
    ...
    // Display the industry information based on the first digit
    outputParagraph.innerHTML +=
    ...
    // Display the digits related to the account number
    outputParagraph.innerHTML +=
